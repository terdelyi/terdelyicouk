      ...
      <form v-on:submit.prevent="handleSubmit" name="contact" action="/contact/thanks/" method="post" data-netlify="true" data-netlify-honeypot="bot-field" autocomplete="off">
        ...
        <div class="sm:flex mb-4">
          <div class="sm:w-1/2 sm:pr-4 mb-4 sm:mb-0">
            ...
            <input type="text" name="name" v-model="formData.name" class="text-input" required>
          </div>
          <div class="sm:w-1/2 sm:pl-4">
            <div class="font-semibold dark:text-grey-400 mb-2 text-sm">Your email address</div>
            <input type="email" name="email" v-model="formData.email" class="text-input" required>
          </div>
        </div>
        <div class="mb-4">
          <div class="font-semibold dark:text-grey-400 mb-2 text-sm">How can I help you?</div>
          <textarea name="message" v-model="formData.message" class="text-input h-32" required></textarea>
        </div>
        <div>
